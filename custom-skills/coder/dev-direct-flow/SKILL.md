...
metadata:
  hermes:
    tags: [dev, coder, direct, micro, interactive, stack-neutral]
...
4. 다음 일반적인 mutation 표현은 DIRECT 승인이 아니다.
   ...
   - `재검토 후 수정해주세요`
   - `오류가 있으면 수정해주세요`
5. 명시적 DIRECT 선택이 없으면 source/plan/grep/read/write/patch/build/test를 시작하지 않는다. 어떤 Stack/Capability Skill도 구현 목적으로 로드하지 않는다.
...
- compile/typecheck 또는 짧은 targeted/affected test로 충분히 검증 가능

대표 후보: 오타/Javadoc/JSDoc/주석, 로그/메시지, 명백한 null/조건문 오류, 작은 validation, 작은 테스트/Markdown/UI text 수정.

다음은 DIRECT가 아니다.

- 여러 호출 흐름을 먼저 분석해야 변경 범위를 알 수 있는 작업
- 기존/신규 흐름 비교, 공통 Utility/Component 재사용 여부 판단처럼 source 확인 전 scope가 불명확한 작업
...
4. 기존 패턴과 공통 Foundation을 따르는 최소 diff만 만든다.
5. 변경 위험과 stack에 맞는 최소 compile/typecheck/lint/affected test 중 필요한 검증을 실행한다.
...
- 테스트/검증 성공을 확인하지 않았으면 성공했다고 보고하지 않는다.
