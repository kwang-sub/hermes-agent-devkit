...
PR_PUBLISH_LIB = ROOT / "custom-skills/orchestrator/git-pr-publish/scripts/pr_publish_lib.py"
...
def run_git(args: list[str], env: dict[str, str]) -> str:
    result = subprocess.run(["git", *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, env=env, check=False)
    if result.returncode != 0:
        raise SystemExit(f"[FAIL] git {' '.join(args)} failed ({result.returncode})\n{result.stdout}\n{result.stderr}")
...
def verify_profile_home_independent_global_config() -> None:
    with tempfile.TemporaryDirectory(prefix="hermes-git-config-") as temp_dir:
        ...
        if run_git(["config", "--global", "--get", "user.name"], env_b) != "DevKit Test":
            raise SystemExit("[FAIL] GIT_CONFIG_GLOBAL user.name did not survive routed-profile HOME isolation")
        if run_git(["config", "--global", "--get", "user.email"], env_b) != "devkit@example.invalid":
            raise SystemExit("[FAIL] GIT_CONFIG_GLOBAL user.email did not survive routed-profile HOME isolation")
...
def main() -> int:
    ...
    require(updater, (
        '[switch]$SkipGitHubAuth', 'function Test-GitHubAuthentication', 'function Initialize-GitPublishing',
        'HERMES_GIT_USER_NAME', 'HERMES_GIT_USER_EMAIL', 'GH_CONFIG_DIR',
        '"config", "--global", "user.name"', '"config", "--global", "user.email"',
        'gh auth status --hostname github.com', '"auth", "login"', '"--git-protocol", "https"',
        'Git publish identity/auth bootstrap', 'GIT_IDENTITY_CONFIGURED=', 'GITHUB_AUTH_READY=',
    ...
    require(sample, (
        ...
        'HERMES_GIT_CONFIG_GLOBAL=/opt/data/gitconfig', 'HERMES_GH_CONFIG_DIR=/opt/data/gh',
        'GitHub 인증 토큰은 .env에 넣지 않습니다.',
    ), "sample.env Git publish configuration")

    require(library, ('HERMES_GH_CONFIG_DIR', '"/opt/data/gh"', 'gh authentication is not ready'),
            "git-pr-publish persistent gh auth contract")

    forbid(sample, ('GH_TOKEN=', 'GITHUB_TOKEN=', 'GITHUB_PAT='), "sample.env secret policy")
    forbid(updater, ('gh auth login --with-token', 'GH_TOKEN=', 'GITHUB_TOKEN='), "update-devkit GitHub auth policy")
    verify_profile_home_independent_global_config()
    print("[PASS] Git publish bootstrap: git-pr-publish + persistent profile-independent Git/GitHub auth contract verified")
    return 0
