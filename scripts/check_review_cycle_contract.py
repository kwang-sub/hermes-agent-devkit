...
def main():
    ...
    require(STANDARD,("/opt/data/shared/references/approval-gate-rules.md","WORKSPACE_APPROVED","BRANCH_APPROVED","MODEL_APPROVED","PLAN_APPROVED","REQUIREMENT_DELTA_APPROVED","Requirement Delta Approval","[추가 요구사항 확인]","clarify","choices","[Project 선택]","[Workspace 선택]","[Branch 선택]","[Coder 모델 선택]","[작업 계획 승인]","DEFAULT | PREMIUM","Reviewer Model은 항상 DEFAULT","flow_model_policy.py resolve","model=<MODEL>","provider=<PROVIDER>","dev-flow-model-policy","Model Escalation: REQUIRE_REAPPROVAL","기존 카드 재작업 계약","Requirement Delta:","Approval Reuse:","migrate-existing","STATUS=legacy-task-migrated","SNAPSHOT_SOURCE=durable-comment","신규/대체 Task 승인 불변식","대체 카드 생성 승인","서로 다른 승인 Gate를 한 질문으로 합치지 않는다","NO_EXTRA_KANBAN_CONFIRMATION","NOTIFY_REGISTRATION_EVENT=queued"),f)
    require(APPROVAL_RULES,("한 번의 사용자 확인에서는 하나의 의사결정만 요청한다","clarify","choices","↑/↓ 이동 + Enter 선택","Other (type your answer)","Workspace와 Branch는 서로 다른 Gate다","[추가 요구사항 확인]","requirement_delta_approved","Requirement Delta","AUTO_DISPATCH","NO_EXTRA_KANBAN_CONFIRMATION"),f)
    require(DISPATCH,("Coder Model Tier(DEFAULT|PREMIUM) 승인 완료","model=MODEL","provider=PROVIDER","dev-flow-model-policy","model_override == MODEL","provider_override == PROVIDER","review-enter","changes-return","Reviewer profile DEFAULT","NOTIFY_REGISTRATION_EVENT=queued","registered","최초 등록 알림"),f)
    ...
    print("[PASS] risk-based review-cycle, model-transition, requirement-delta approval, registration notification, and auto-dispatch invariants"); return 0
