...
→ API_SPEC_APPROVAL (계약 변경으로 REQUIRED가 된 경우)
...
- Acceptance Criteria, 주요 구현 방식, API contract, 롤백 범위, 금지 작업이 바뀜
...
- Requirement Delta 승인 뒤에는 영향받는 기존 승인만 REUSE/REQUIRED 판정하고, API contract가 바뀌면 API Spec Gate를 다시 수행한 뒤 갱신된 Implementation Plan을 별도로 승인받는다.
...
Orchestrator는 `dev-api-spec`의 Markdown 형식으로 API Spec DRAFT를 먼저 일반 메시지로 보여준다. source/config는 수정하지 않는다.
...
question:
  [API 규격 승인]
  위 Markdown API Specification을 구현 기준으로 확정할까요?
choices:
  - 규격 승인
  - 규격 보류
```
...
처리 규칙:
- `규격 승인`: `API_SPEC_APPROVED=true`, `API_SPEC_STATUS=APPROVED`.
- `규격 보류`: 구현/dispatch 금지.
- `Other`: API 규격 수정 요구로 처리하고 DRAFT를 갱신한 뒤 같은 `[API 규격 승인]` Gate를 다시 수행한다.
- Method/Path/Request/Response/Error/Auth/Nullability/Money/Date 등 의미 계약이 바뀌면 기존 API Spec 승인은 무효다.
- API Spec 승인과 Plan 승인을 한 질문으로 합치지 않는다.
- `SOURCE_SYNC | AUDIT | NOT_REQUIRED`는 이 Gate를 만들지 않고 `API_SPEC_APPROVAL=NOT_REQUIRED`로 둔다.
...
신규 요청은 `requirement_delta_approved=not_required`다. `DESIGN_FIRST` API 변경이면 `api_spec_approved`가 반드시 true여야 하고, `SOURCE_SYNC | AUDIT | NOT_REQUIRED`는 `not_required`다.
...
하나라도 필요한 승인이 없으면 `prepare_dispatch.py`, `kanban_create`, `kanban_unblock`, worker dispatch를 실행하지 않는다.

## 재승인

```text
추가 요구사항/롤백/범위 교체 → Requirement Delta Gate + 필요 시 API Spec Gate + Plan Gate 재승인
API contract 변경 → API Spec Gate + Plan Gate 재승인
...
Goal/Acceptance Criteria/주요 구현 계획 변경 → Requirement Delta Gate + 필요 시 API Spec Gate + Plan Gate 재승인
```
