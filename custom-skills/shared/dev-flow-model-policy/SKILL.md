...
metadata:
  hermes:
    tags: [dev, flow, model, approval, coder, reviewer, kanban, direct-flow, standard-flow]
    related_skills: [dev-workflow-orchestrate, dev-workspace-dispatch, dev-implement-plan, dev-code-review, dev-review-cycle]
...
- Coder 모델은 신규 Direct/Standard dispatch 전에 사용자가 명시적으로 승인한다.
- 실행 방식 선택과 Coder 모델 선택은 별도 Gate다.
...
Direct와 Standard 모두 Reviewer를 필수로 사용하며 risk LOW self-complete 예외는 없다.
...
`done`/`archived` Task는 migration하지 않는다. `MODEL_POLICY_SNAPSHOT_V1` durable Kanban comment가 승인 snapshot을 보존한다.
...
Direct/Standard 구현 완료 시 Coder는 항상 Reviewer에게 인계한다.
...
handoff evidence를 준비한 뒤 claim-bound Hermes lifecycle 도구를 호출한다.
...
따라서 Worker가 별도 shell helper로 model override를 먼저 변경하면 안 된다. `kanban_request_review`가 실패하면 임의 재호출/수동 env 주입 대신 해당 오류를 근거로 `kanban_block(kind=capability)`한다.
