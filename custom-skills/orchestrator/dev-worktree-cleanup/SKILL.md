---
name: dev-worktree-cleanup
description: merge가 확인된 linked Git worktree를 사용자 승인 후 제거하고 local branch 및 선택한 remote branch를 안전하게 정리한다.
version: 0.1.0
author: local
platforms: [linux]
metadata:
  hermes:
    tags: [dev, git, worktree, cleanup, branch, pull-request, github, approval]
    related_skills: [dev-pr-publish, dev-workspace-dispatch]
    requires_tools: [terminal, clarify]
---

# dev-worktree-cleanup

PR/branch 작업이 끝난 뒤 남은 **linked worktree + branch refs**를 정리하는 post-merge cleanup Skill이다. source 수정, merge, reset, stash를 수행하지 않으며, merge 완료가 증명된 작업만 사용자 승인 후 정리한다.

표준 흐름:

```text
PR merged / branch merged
→ /dev-worktree-cleanup
→ read-only cleanup preflight
→ [Worktree 정리 Preview]
→ [Worktree 정리 승인]
→ worktree remove
→ exact local branch ref delete
→ worktree prune
→ 선택한 경우 remote branch delete
→ 결과 보고
```

## 1. 진입 조건

다음을 모두 만족해야 한다.

- 대상은 **primary worktree가 아닌 linked worktree**다.
- detached HEAD가 아니다.
- worktree가 Git metadata에 정확히 1개로 등록되어 있다.
- worktree가 `locked` 또는 `prunable` 상태가 아니다.
- tracked/untracked 변경이 전혀 없다.
- 같은 branch가 다른 worktree에도 연결되어 있지 않다.
- local HEAD와 존재하는 remote branch HEAD가 동일하다.
- open Pull Request가 남아 있지 않다.
- 아래 Merge Evidence 중 하나가 존재한다.

```text
1. GitHub merged PR + PR head SHA == local HEAD
2. local HEAD가 resolved base ref의 ancestor
```

첫 번째 Evidence는 squash/rebase merge처럼 feature commit이 base history의 ancestor가 아닌 경우를 지원한다.

변경 파일이 하나라도 있으면 `--force`, reset, stash, clean으로 우회하지 않고 BLOCK한다.

## 2. Base Branch 결정

Base Branch는 추측하지 않는다. 우선순위는 다음과 같다.

```text
1. 사용자가 명시한 base branch
2. 현재 HEAD와 정확히 일치하는 merged PR의 base branch
3. refs/remotes/<remote>/HEAD
4. 그래도 불명확하면 사용자에게 clarify
```

Base Branch와 cleanup 대상 branch가 같으면 BLOCK한다.

## 3. Read-only Cleanup Preflight

먼저 다음 helper를 실행한다.

```bash
python3 "${HERMES_SKILL_DIR}/scripts/prepare_cleanup.py" \
  --workspace "<WORKTREE>" \
  --remote "<REMOTE>" \
  [--base-branch "<BASE_BRANCH>"]
```

helper는 mutation 없이 다음을 확정한다.

```text
Primary worktree
Target worktree
Branch / HEAD SHA
Base branch / base ref
Remote URL
Remote branch 존재 여부와 SHA
GitHub auth 상태
Merged PR URL/번호 (있을 때)
Merge Evidence
Remote branch 삭제 가능 여부
Cleanup fingerprint
```

`CLEANUP_FINGERPRINT`는 approval snapshot이다. 승인 후 다음 중 하나라도 바뀌면 이전 승인은 무효다.

```text
worktree registration
branch / HEAD
working tree status
base branch/ref
remote branch SHA
merged PR evidence
```

변경되면 preflight부터 다시 수행하고 Preview/Gate를 다시 보여준다.

## 4. Worktree 정리 Preview

`clarify` 전에 일반 메시지로 다음을 보여준다.

```text
[Worktree 정리 Preview]
Worktree: <path>
Branch: <branch>
HEAD: <sha>
Base: <base>
Merge Evidence: <github-pr-merged | git-ancestor>
PR: <url | none>
Remote Branch: <exists | already absent>

정리 대상:
- linked worktree 제거
- local branch 제거
- remote branch: <삭제 | 유지 | 이미 없음>
```

Windows/IntelliJ가 대상 Worktree를 열고 있으면 파일 lock으로 `git worktree remove`가 실패할 수 있으므로 가능하면 프로젝트를 닫고 승인하도록 안내한다. 실패해도 force 삭제하지 않는다.

## 5. Gate — Worktree 정리 승인

Remote branch가 존재하고 삭제 가능한 경우:

```text
question:
  [Worktree 정리 승인]
  병합이 확인된 Worktree와 선택한 Branch refs를 정리할까요?
choices:
  - Worktree + 로컬/원격 Branch 정리
  - Worktree + 로컬 Branch만 정리
  - 취소
```

Remote branch가 이미 없으면:

```text
question:
  [Worktree 정리 승인]
  병합이 확인된 Worktree와 로컬 Branch를 정리할까요?
choices:
  - 정리 승인
  - 취소
```

한 Gate에서는 cleanup scope 하나만 결정한다. `Other`에서 base/scope 변경 요구가 들어오면 승인으로 간주하지 않고 preflight부터 다시 수행한다.

## 6. 승인 후 Mutation

승인 후 helper를 **정확히 한 번** 실행한다.

Remote branch 유지:

```bash
python3 "${HERMES_SKILL_DIR}/scripts/cleanup_worktree.py" \
  --workspace "<WORKTREE>" \
  --remote "<REMOTE>" \
  --base-branch "<BASE_BRANCH>" \
  --fingerprint "<CLEANUP_FINGERPRINT>"
```

Remote branch까지 삭제:

```bash
python3 "${HERMES_SKILL_DIR}/scripts/cleanup_worktree.py" \
  --workspace "<WORKTREE>" \
  --remote "<REMOTE>" \
  --base-branch "<BASE_BRANCH>" \
  --fingerprint "<CLEANUP_FINGERPRINT>" \
  --delete-remote
```

helper 순서는 다음과 같다.

```text
fingerprint + merge evidence 재검증
→ git worktree remove <worktree>
→ target worktree 미등록 확인
→ git update-ref -d refs/heads/<branch> <expected HEAD>
→ git worktree prune
→ remote 삭제 선택 시 remote SHA 재확인
→ git push <remote> --delete <branch>
```

Local branch는 `git branch -D`를 사용하지 않는다. squash/rebase merge도 지원하면서 잘못 이동한 branch를 삭제하지 않기 위해 **승인된 exact HEAD를 old SHA로 지정한 `git update-ref -d`**만 사용한다.

## 7. Remote Branch 삭제 규칙

Remote branch 삭제는 명시적으로 해당 choice를 승인했을 때만 수행한다.

삭제 직전에 `git ls-remote`로 remote branch SHA를 다시 읽고 승인 시점 SHA와 다르면 remote branch를 보존하고 `partial`로 종료한다.

HTTPS GitHub remote에서는 기존 `dev-pr-publish`와 같은 persistent `gh` 인증을 one-command credential helper로 사용한다. GitHub 인증이 준비되지 않으면 remote 삭제 option을 제공하지 않는다.

GitHub에서 PR merge 후 branch를 자동 삭제해 이미 remote ref가 없으면 성공 상태로 간주하고 local cleanup만 수행한다.

## 8. Partial Failure

Mutation은 rollback을 시도하지 않는다.

```text
worktree remove 실패
→ branch refs는 건드리지 않고 BLOCK

worktree remove 성공 + local ref delete 실패
→ STATUS=partial
→ worktree 제거 상태와 남은 local branch를 보고

local cleanup 성공 + remote delete 실패
→ STATUS=partial
→ local cleanup 완료 상태와 남은 remote branch를 보고
```

Partial 상태에서는 자동 재시도하지 않는다. 현재 상태를 다시 preflight해 다음 조치를 결정한다.

## 9. 절대 금지

```text
git worktree remove --force
git branch -D
git push --force
git push --force-with-lease
git reset
git restore
git clean
git stash
rm -rf
primary worktree 삭제
open PR branch 삭제
merge evidence 없는 branch 삭제
승인되지 않은 remote branch 삭제
```

## 10. dev-pr-publish와 역할 분리

```text
dev-pr-publish
  commit + push + PR create까지만 담당
  PR merge는 사용자가 GitHub에서 수행

dev-worktree-cleanup
  merge 완료 후 worktree/branch cleanup만 담당
```

따라서 PR 생성 직후 자동 cleanup하지 않는다. 사용자가 GitHub에서 merge를 완료한 뒤 별도 요청으로 실행한다.
