...
metadata:
  hermes:
    ...
    requires_tools: [terminal, skill_view, kanban_show, kanban_request_review, kanban_block, kanban_heartbeat]
---
...
6. Task에 `Project Pattern Summary`, `Pattern References`, `Applicable Skills`, `Pattern Conflicts`가 있으면 이를 실제 source와 대조한다. 오래되었거나 불일치하면 source evidence를 우선하되, 승인 scope를 바꿀 정도면 수정하지 말고 BLOCKED한다.
7. `Applicable Skills`에 나열된 Coder capability Skill은 구현 전에 각각 `skill_view("<skill-name>")`으로 전체 본문을 반드시 로드한다. Skill metadata/description만 읽고 세부 규칙을 추측하지 않는다. 존재하지 않거나 로드할 수 없는 Skill이면 해당 Skill이 correctness에 필요한 경우 BLOCKED한다.
8. Spring/Spring Boot 프로젝트를 실제 build/source evidence로 확인하면 Task에 누락되어 있어도 `skill_view("dev-spring-guidelines")`를 호출해 기본 적용한다. 요청/diff가 명백히 해당하면 다음 capability도 자동 감지하여 실제 본문을 로드한다.
   - Controller/Service/DTO/Validation/Exception/API feature → `skill_view("dev-spring-feature")`
   - JPA/Entity/Repository/DataJPA/QueryDSL/Converter/Paging → `skill_view("dev-spring-data")`
   - Spring/JPA 테스트 → `skill_view("dev-spring-test")`
   - OpenAPI/Swagger/Postman → `skill_view("dev-api-docs")`
9. 동일 Skill은 한 worker attempt에서 중복 로드하지 않는다. Standard Flow의 `Applicable Skills`를 우선 재사용하고 누락된 capability만 source evidence로 보완한다.
10. Spring/JPA 데이터 조회에서는 단순 조회는 Spring Data JPA Method Query를 우선하고, 복잡하거나 동적인 조회는 QueryDSL을 우선한다. Native Query는 Method Query/QueryDSL로 해결하기 어려운 근거가 있을 때만 사용하고 이유를 handoff에 기록한다.
11. API 응답은 프로젝트에 존재하는 공통 응답 규격을 사용한다. 공통 규격이 없는 기존 API를 임의로 새로운 wrapper로 변경하지 않는다.
12. Stack/Capability Skill은 기존 project pattern과 공통 Coding Rules를 확장할 뿐 약화하지 않는다. dependency, architecture, 공통 response/error contract를 임의로 새로 도입하지 않는다.
13. targeted verification부터 실행하고 `git diff --check` 및 `scripts/change_summary.py`로 tracked/untracked/status를 수집한다.
14. 정확한 command/result와 검증된 `BASE_SHA`, 사용한 Pattern References/Capability Skills, residual risk를 기록하고 configured `reviewer`에게 `kanban_request_review`만 호출한 뒤 멈춘다. 구현 완료 상태에서 `kanban_complete` 또는 review 대용 `kanban_block`을 호출하지 않는다.
...
Capability Skills Loaded via skill_view
Query Strategy (해당 시)
